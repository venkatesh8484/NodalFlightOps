// ============================================
// Explorer tab — "Ontology Graph Explorer"
// ============================================
// Renders the FLIGHTOPS ONTOLOGY — the class-level taxonomy produced by the OntologyEngine
// pipeline (src/data/04_thesaurus.json + 05_ontology.json) — not the live per-instance KG.
// Object Types (Airport, Airline, Crew, Flight Route, Aircraft, Flight, Telemetry Event,
// Action Type, Action Execution, Aircraft Assignment, ...) are expandable hub nodes; their
// narrower thesaurus terms are satellites; cross-class edges come from the ontology's
// declared Object_Properties. This mirrors PostalOps' "Ontology Explorer" tab exactly —
// same generation mechanism, same rendering approach — so both apps show the actual
// semantic schema instead of a raw dump of live records.
//
// (The live KG instance graph — Airport/FlightRoute/Flight/Aircraft records and their FK
// edges — is a different concern; see src/lib/knowledgeGraph.js#getAllEdges(), consumed by
// the Console tab and the chat's agent context, not by this view.)

import React, { useEffect, useMemo, useRef, useState } from 'react';
import * as d3 from 'd3';
import { Search, ZoomIn, ZoomOut, Maximize2, X, ChevronLeft, ChevronRight } from 'lucide-react';
import {
  ONTOLOGY_CLASSES,
  ONTOLOGY_RELATIONS,
  getIconPath,
...
// ─── Detail panel ───────────────────────────────────────────────────────────
function DetailPanel({ node, onSelect }) {
  if (!node) {
    return (
      <div style={{ padding: '16px', fontSize: '12px', color: 'var(--muted-2)' }}>
        Click a class to inspect its scope note, schema, and relationships. Double-click an
        expandable class (marked ⊕/⊖) to reveal or hide its narrower terms.
      </div>
    );
  }
  const cls = ONTOLOGY_CLASSES[node.id];
  if (!cls) return null;
  const outgoing = ONTOLOGY_RELATIONS.filter((r) => r.source === node.id);
  const incoming = ONTOLOGY_RELATIONS.filter((r) => r.target === node.id && r.source !== node.id);
  return (
    <div style={{ padding: '14px', overflowY: 'auto', height: '100%', boxSizing: 'border-box' }}>
      <div style={{ display: 'flex', alignItems: 'center', gap: '8px', marginBottom: '6px' }}>
        <span
          style={{
            width: '10px', height: '10px', borderRadius: '50%', flexShrink: 0,
            background: cls.metaType === 'ACTION' ? '#10b981' : cls.metaType === 'AUTO_RECORD' ? '#a855f7' : '#3b82f6',
          }}
        />
        <div>
          <div style={{ fontSize: '13px', fontWeight: 700, color: 'var(--ink)' }}>{cls.name}</div>
          <div style={{ fontSize: '10.5px', color: 'var(--muted-2)', textTransform: 'uppercase', letterSpacing: '0.04em' }}>
            {cls.metaType.replace(/_/g, ' ')}{cls.parentClass ? ` · subclass of ${cls.parentClass}` : ''}
          </div>
        </div>
      </div>

      {cls.desc && (
        <p style={{ fontSize: '11.5px', color: 'var(--muted)', lineHeight: 1.5, margin: '8px 0' }}>{cls.desc}</p>
      )}

      {cls.aliases.length > 0 && (
        <div style={{ fontSize: '10.5px', color: 'var(--muted-2)', marginBottom: '10px' }}>
          Also known as: {cls.aliases.join(', ')}
        </div>
      )}

      <div style={{ fontSize: '11px', fontWeight: 700, color: 'var(--muted)', textTransform: 'uppercase', letterSpacing: '0.05em', margin: '10px 0 6px' }}>
        Schema Properties
      </div>
        ...
        <tbody>
          {cls.properties.map((p) => (
            <tr key={p.name} style={{ borderBottom: '1px solid var(--line)' }}>
              <td style={{ padding: '4px 6px 4px 0', color: 'var(--ink)', fontWeight: 600, verticalAlign: 'top', whiteSpace: 'nowrap' }}>{p.name}</td>
              <td style={{ padding: '4px 0', color: 'var(--muted)' }}>{p.desc} <span style={{ color: 'var(--muted-2)' }}>({p.type})</span></td>
            </tr>
          ))}
        </tbody>
      </table>
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: '4px' }}>
            {outgoing.map((r) => (
              <button
                ...
                style={{
                  textAlign: 'left', fontSize: '10.5px', padding: '5px 7px', borderRadius: '6px',
                  border: '1px solid var(--line)', background: 'var(--surface)', color: 'var(--ink)', cursor: 'pointer',
                }}
              >
                <span style={{ color: r.type === 'action' ? '#10b981' : '#2563eb', fontFamily: 'var(--mono)' }}>{r.label}</span>
                {' → '}{r.target}
              </button>
            ))}
            {incoming.map((r) => (
...
export default function ExplorerTab() {
  ...
  useEffect(() => {
    ...
    const simulation = d3
      ...
      // Containment: forceCenter alone only shifts the centre of mass, it does not
      // stop the graph spreading past the canvas. Without these two, 10 of the 34
      // classes drifted outside the viewport. Matches PostalOps' 0.08 strength.
    ...
    // Invisible wide hit areas. A 1.2px stroke is effectively impossible to hover
    // with a mouse, so every link gets a transparent 12px-wide twin that carries
    // the pointer events instead. Drawn in the same layer, under the nodes.
    const linkHitAreas = linkLayer
    ...
    linkHitAreas
      .on('mouseover', (event, d) => {
        ...
        linkElements.filter((l) => l.id !== d.id).style('opacity', 0.15);
        linkLabelElements.style('opacity', (l) => (l.id === d.id ? 1 : 0));
      })
      .on('mouseout', () => resetHighlight());

    simulation.on('tick', () => {
      const linkPath = (d) => `M${d.source.x},${d.source.y} L${d.target.x},${d.target.y}`;
